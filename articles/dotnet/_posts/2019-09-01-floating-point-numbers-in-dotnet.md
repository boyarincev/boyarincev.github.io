...
Это не самостоятельная статья, в том плане, что вряд ли вы сможете разобраться с числами с плавающей точкой, прочитав одну её, поэтому я порекомендую ссылки на другие статьи (хотя и их тоже не назовёшь идеальными).
...
В этой статье я больше сосредоточюсь на конкретных байтовых представлениях чисел в памяти, разобрав как они получаются. Мне кажется, таких конкретных примеров не хватает в статьях выше для лучшего понимания.
...
В Double для мантиссы отводятся 52 разряда, ещё один старший разряд в нормализованных числах подразумевается и он всегда равен 1. __Чтобы из мантиссы хранимой в Double получить настоящую мантиссу, которую нужно умножать на экспоненту, нужно добавить к ней ещё один разряд заполненный 1 и разделить на 2 в степени 52. __
...
Запустить его в браузере вы можете с помощью try.dot.net, перейдя по [этой ссылке](https://try.dot.net/?fromGist=c6e6f2f0b3aa672702d3f274ccbcebc6) и самостоятельно сделать разбор любого нормализованного числа.
